  ...
  if action then
    ...
    if status then
      ...
      elseif type(result) == 'boolean' then
        ...
        warn("Nieprawidłowy zwrot z akcji cavebot (" .. currentAction.action .. "), powinno byc \"retry\", false or true, jest: " .. tostring(result))
      end
    else
      warn("ostrzegaj podczas wykonywania akcji cavebot (" .. currentAction.action .. "):\n" .. result)
    end    
  else
...
CaveBot.setCurrentProfile = function(name)
  if not g_resources.fileExists("/bot/"..botConfigName.."/cavebot_configs/"..name..".cfg") then
    return warn("nie ma profilu cavebota o takiej nazwie!")
  end
  CaveBot.setOff()
